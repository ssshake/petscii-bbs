package eu.sblendorio.bbs.core;

public class Keys {
    public static final int F1 = 133;
    public static final int F3 = 134;
    public static final int F5 = 135;
    public static final int F7 = 136;
    public static final int F2 = 137;
    public static final int F4 = 138;
    public static final int F6 = 139;
    public static final int F8 = 140;

    public static final int UP = 145;
    public static final int DOWN = 17;
    public static final int LEFT = 157;
    public static final int RIGHT = 29;

    public static final int RETURN = 13;
    public static final int HOME = 19;
    public static final int CLR = 147;
    public static final int DEL = 20;
    public static final int INS = 148;
    public static final int SPACE_CHAR = 32;
    public static final int PERIOD = 46;

    public static final int LOWERCASE = 14;
    public static final int UPPERCASE = 142;

    public static final int REVON = 18;
    public static final int REVOFF = 146;
    public static final int CASE_LOCK = 8;
    public static final int CASE_UNLOCK = 9;
}
